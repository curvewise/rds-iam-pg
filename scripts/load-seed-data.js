...
;(async () => {
  ...
  await client.query('INSERT INTO measured_body_views VALUES ($1, $2, $3)', [
    measured_body_views_id,
    anonymized_female_views.version,
    anonymized_female_views.isPerspective,
  ])
  let id = 0
  for (const measurement of anonymized_female_views.measurements) {
    ...
    await client.query(
      ...
      [id, measurement.name, measurement.index, measured_body_views_id]
    )
    await client.query(
